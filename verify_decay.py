...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if global_step % 20 == 0:
            optimizer.zero_grad()
            nat_loss = F.cross_entropy(model(data), target)
            nat_loss.backward()
            nat_grad = {}
            ...
            for name  in model.state_dict():
                if 'running_var' in name:
                    ...
                    writer.add_scalar(name, state, global_step=global_step)
...
def eval_adv(model, device, test_loader, epoch):
    ...
    writer.add_scalar(
        'nat_acc', 1 - natural_err_total / total, global_step=global_step)
    writer.add_scalar(
        'adv_acc', 1 - robust_err_total / total, global_step=global_step)
...
def main():
    ...
    for epoch in range(init_epoch, args.epochs + 1):
        ...
        print('======================================')
        eval_adv(model, device, test_loader, epoch)
        print('======================================')
