...
from tiny_imagenet_loader import get_loader
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        a_epsilon = args.epsilon
        a_step_size = args.step_size
        a_num_steps = args.num_steps #random.randint(0,args.num_steps)
        ...
        if args.train_nat:
            ...
            loss, x_adv = adv_loss_func(
                ...
                step_size=a_step_size, epsilon=a_epsilon, perturb_steps=a_num_steps)
            #all_epsilons, _ = torch.max(torch.abs(data-x_adv).view(128,-1), dim=-1)
            #avg_epsilon = torch.mean(all_epsilons)
            #writer.add_scalar('avg_epsilon', avg_epsilon, global_step=global_step)
...
def main():
    ...
    model = WideResNet(widen_factor=args.width, extra_stride=2, num_classes=200).to(device)
